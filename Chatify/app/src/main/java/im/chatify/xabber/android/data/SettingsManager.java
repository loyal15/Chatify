 ...
 */
package im.chatify.xabber.android.data;
...
import im.chatify.R;
import im.chatify.page.chat.ComparatorByName;
import im.chatify.page.chat.ComparatorByStatus;
import im.chatify.xabber.android.data.account.AccountManager;
import im.chatify.xabber.android.data.account.StatusMode;
import im.chatify.xabber.android.data.connection.NetworkManager;
import im.chatify.xabber.android.data.extension.attention.AttentionManager;
...
import im.chatify.xabber.android.utils.Emoticons;
import im.chatify.xabber.xmpp.carbon.CarbonManager;
...
public class SettingsManager implements OnInitializedListener,
    ...
    private static SharedPreferences getSharedPreferences() {
        return PreferenceManager.getDefaultSharedPreferences(CIApp
                .getInstance());
    }
    ...
    private static int getInt(int key, int def) {
        ...
        try {
            ...
            return Integer.parseInt(CIApp.getInstance().getString(def));
        }
    }

    private static boolean getBoolean(int key, boolean def) {
        return getSharedPreferences().getBoolean(
                CIApp.getInstance().getString(key), def);
    }

    private static boolean getBoolean(int key, int def) {
        return getBoolean(key, CIApp.getInstance().getResources()
                .getBoolean(def));
    }

    private static void setBoolean(int key, boolean value) {
        Editor editor = getSharedPreferences().edit();
        editor.putBoolean(CIApp.getInstance().getString(key), value);
        editor.commit();
    }
    ...
    private static String getString(int key, int def) {
        return getString(key, CIApp.getInstance().getString(def));
    }

    private static void setString(int key, String value) {
        Editor editor = getSharedPreferences().edit();
        editor.putString(CIApp.getInstance().getString(key), value);
        editor.commit();
    }
    ...
    private static Uri getSound(int key, Uri defaultUri, int defaultResource) {
        String defaultValue = CIApp.getInstance().getString(
                defaultResource);
...
//    }
    ...
    public static Map<Pattern, Integer> interfaceSmiles() {
        ...
        if (CIApp.getInstance().getString(R.string.interface_smiles_none_value).equals(value)) {
            return Collections.unmodifiableMap(Emoticons.NONE_EMOTICONS);
        } else if (CIApp.getInstance().getString(R.string.interface_smiles_android_value).equals(value)) {
    ...
    public static int bootCount() {
        return getSharedPreferences()
                .getInt(CIApp.getInstance().getString(
    ...
    public static void incrementBootCount() {
        Editor editor = getSharedPreferences().edit();
        editor.putInt(
                CIApp.getInstance().getString(R.string.boot_count_key),
                bootCount() + 1);
        editor.commit();
    }
    ...
    public void onMigrate(int toVersion) {
        switch (toVersion) {
            ...
            case 40:
                ...
                try {
                    if (getBoolean(R.string.chats_show_status_change_key, false))
                        value = CIApp.getInstance().getString(
            ...
            case 45:
                ...
                break;
            case 65:
                SharedPreferences settings = CIApp.getInstance()
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences,
                                          String key) {
        if (key.equals(CIApp.getInstance().getString(
                R.string.chats_show_status_change_key))) {
            MessageManager.getInstance().onSettingsChanged();
        } else if (key.equals(CIApp.getInstance().getString(
