...
import im.chatify.xabber.android.data.account.AccountProtocol;
import im.chatify.xabber.android.data.account.OAuthManager;
import im.chatify.xabber.android.data.account.OAuthResult;
...
public class ConnectionThread implements
    ...
    public ConnectionThread(final ConnectionItem connectionItem) {
        ...
        started = false;
        attributes = connectionSettings.getAttributes();
    }
    ...
    private void onReady(XMPPTCPConnectionConfiguration.Builder builder) {
...

        {
            try {
                ...
                MemorizingTrustManager mtm = new MemorizingTrustManager(CIApp.getInstance());
...
//        }
    ...
    private void registerAccount(final String password) {
        try {
            if (attributes != null)
                createAccount(login, password, attributes);
            else
    ...
    public void preprocessForRetrievePassword() throws SmackException.NotConnectedException {
        ...
        reg.setStanzaId("GGGG-1");
        reg.setTo(xmppConnection.getServiceName());

        xmppConnection.createPacketCollectorAndSend(new StanzaFilter() {
            @Override
    ...
    public void retrievePassword() throws SmackException.NotConnectedException {
...
//        attributes.put("code", "");
    ...
    public void createAccount(String username, String password) throws SmackException.NoResponseException, XMPPException.XMPPErrorException, SmackException.NotConnectedException {
        ...
        for (String attributeName : AccountManager.getInstance(xmppConnection).getAccountAttributes()) {
...
//        }
    ...
    private void onAuthorized() {
        ...
        if (connectionItem instanceof AccountItem) {
            im.chatify.xabber.android.data.account.AccountManager.getInstance().removeAuthorizationError(
    ...
    public void connectionClosedOnError(final Exception e) {
        ...
        if (SettingsManager.showConnectionErrors()) {
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(CIApp.getInstance(),
                            CIApp.getInstance().getString(R.string.CONNECTION_FAILED) + ": " + e.getMessage(),
                            Toast.LENGTH_SHORT
                    ).show();
                }
            });
