...
import im.chatify.xabber.android.data.SettingsManager.ChatsShowStatusChange;
...
import im.chatify.xabber.android.data.account.OnAccountDisabledListener;
...
import im.chatify.xabber.android.data.roster.OnStatusChangeListener;
...
import im.chatify.xabber.xmpp.carbon.CarbonManager.Direction;
...
public class MessageManager implements OnLoadListener, OnPacketListener, OnDisconnectListener,
        ...
        CIApp.getInstance().runOnUiThread(new Runnable() {
            ...
            public void run() {
                for (OnChatChangedListener onChatChangedListener
                        : CIApp.getInstance().getUIListeners(OnChatChangedListener.class)) {
     ...
     * @param text
     */
    public void sendMessage(String account, String user, String latitude, String longitude, String country, String locality, String text) {
        AbstractChat chat = getChat(account, user);
        if (chat == null) {
            chat = createChat(account, user);
        }
        MessageItem messageItem = chat.newMessage(latitude, longitude, country, locality, text);
        chat.sendQueue(messageItem);
    }
