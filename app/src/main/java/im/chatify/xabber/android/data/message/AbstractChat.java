...
import im.chatify.xabber.xmpp.LocationPacketExtention;
...
public abstract class AbstractChat extends BaseEntity {
     ...
     */
    private static final String NO_RECORD_TAG = "im.chatify.xabber.android.data.message.NO_RECORD_TAG";
    ...
    //Added by Loyal
    abstract protected MessageItem newMessage(String latitude, String longitude, String country, String locality, String text);
     ...
     * @return New message packet to be sent.
     */
    public Message createMessagePacket(String latitude, String longitude, String country, String locality, String body) {
        Message message = new Message();
        message.setTo(getTo());
        message.setType(getType());
        message.setBody(body);
        message.addExtension(new LocationPacketExtention(latitude, longitude, country, locality));
        message.setThread(threadId);
        return message;
    }
    ...
    protected void sendQueue(MessageItem intent) {
        ...
        for (final MessageItem messageItem : sendQuery) {
            ...
            if (text == null) {
                ...
                //Added by Loyal
                Message message;
                String latitude = prepareText(messageItem.getLatitude());

                if(latitude == null) {
                    message = createMessagePacket(text);
                    messageItem.setPacketID(message.getPacketID());
                }else {
                    String longitude = prepareText(messageItem.getLongitude());
                    String country = prepareText(messageItem.getCountry());
                    String locality = prepareText(messageItem.getLocality());

                    message = createMessagePacket(latitude, longitude, country, locality, text);
                    messageItem.setPacketID("msg_1");
                }
                //////////////////
